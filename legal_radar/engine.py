...
_SLANG_MAP: dict[str, str] = {
    ...
    "flex": "khoe khoang",
    "cancel": "tẩy chay",
    "dox": "rò rỉ thông tin cá nhân",
    "doxxing": "rò rỉ thông tin cá nhân",
    "stalking": "theo dõi",
    ...
    "ib": "nhắn tin riêng",
    "ibox": "nhắn tin riêng",
    "stalker": "người theo dõi",
    "hater": "người ghét bỏ",
    "anti": "người phản đối",
    "simp": "người hâm mộ cuồng nhiệt",
    ...
    "slay": "xuất sắc",
    "ate": "làm tốt",
    "understood the assignment": "hiểu bài",
    "periodt": "vậy thôi",
    ...
    "bão": "cuộc tấn công",
    "bay màu": "bị xóa",
    "bị report": "bị báo cáo",
    "bị ban": "bị cấm",
    "khóa mõm": "bị cấm phát biểu",
    "bị khóa mõm": "bị cấm phát biểu",
    "check": "kiểm tra",
    "verify": "xác minh",
    "confirm": "xác nhận",
    "update": "cập nhật",
...
def phan_loai_claim(
    ...
    citations = [_build_citation(dk) for dk in matched_dks[:2]]
...
def diff_thay_the(dk_id: str, kg: KnowledgeGraph) -> str | None:
    ...
    edge = edges[0]
    diff_text = edge.thuoc_tinh.get("diff", "")
    moc = edge.thuoc_tinh.get("moc_thoi_gian", "")

    old_id = edge.source
    new_id = edge.target
    old_node = kg.find_node(old_id)
    new_node = kg.find_node(new_id)

    parts = []
    if old_node and new_node:
        old_vb = kg.find_node(old_node.van_ban) if isinstance(old_node, DieuKhoan) else None
        new_vb = kg.find_node(new_node.van_ban) if isinstance(new_node, DieuKhoan) else None
        if old_vb and new_vb:
            parts.append(f"{old_vb.so_hieu} → {new_vb.so_hieu}")

    if old_node and new_node and isinstance(old_node, DieuKhoan) and isinstance(new_node, DieuKhoan):
    ...
    if moc:
        parts.append(f"Hiệu lực từ {moc}")
