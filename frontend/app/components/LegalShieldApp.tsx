...
export function LegalShieldApp() {
  ...
  const data = useMemo(
    ...
    [query, sortDesc, statusById, statusFilter, verdictFilter],
  );
    ...
    <div className="monitor-app">
      <aside className="monitor-sidebar">
        ...
        <nav aria-label="Điều hướng chính">
          <button className="active"><span>▦</span> Hàng đợi giám sát <b>{cases.filter((x) => (statusById[x.id] ?? x.status) !== "Đã xử lý").length}</b></button>
...
function Queue({
  rows, verdictFilter, statusFilter, sortDesc, onVerdictFilter, onStatusFilter, onSort, onOpen,
}: {
  rows: Case[];
  ...
  const openCount = cases.filter((item) => item.status !== "Đã xử lý").length;
  return (
    <div className="monitor-page">
      <div className="queue-heading">
        <div><span className="eyebrow">ĐIỀU PHỐI NỘI DUNG</span><h1>Hàng đợi giám sát</h1><p>Rà soát claim do AI phát hiện và chuyển hồ sơ tới chuyên viên phụ trách.</p></div>
      ...
      <section className="queue-card">
        <div className="queue-toolbar">
          ...
          <button className="sort-button" onClick={onSort}>↕ Mức ưu tiên: {sortDesc ? "cao trước" : "thấp trước"}</button>
        </div>
        ...
        <footer className="queue-footer">Hiển thị <strong>{rows.length}</strong> / {cases.length} hồ sơ <span>Dữ liệu mock · cập nhật 17/07/2026, 10:46</span></footer>
      </section>
    </div>
  );
}
